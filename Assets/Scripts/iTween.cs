...
public class iTween : MonoBehaviour
{
	...
	private void GenerateShakePositionTargets()
	{
		...
		if (isLocal)
		{
			vector3s[0] = base.transform.localPosition;
		}
		else
		{
	...
	{
		Debug.LogError("CameraFade is no longer supported in iTween!");
	}
	...
	public static int Count(GameObject target)
	{
		...
		{
			int value = 0;
			tweenCount.TryGetValue(target.GetInstanceID(), out value);
			return value;
		}
		Component[] components = target.GetComponents<iTween>();
	...
	private void OnDestroy()
	{
		...
		if (tweenCount.ContainsKey(instanceID))
		{
			int num = tweenCount[instanceID];
			int num2 = num - 1;
			if (num2 == 0)
			{
				tweenCount.Remove(instanceID);
			}
			else
			{
				tweenCount[instanceID] = num2;
			}
		}
	}
	...
	private void Awake()
	{
		...
		if (OPTIMIZED)
		{
			...
			if (!tweenCount.ContainsKey(instanceID))
			{
				tweenCount.Add(instanceID, 0);
			}
			tweenCount[instanceID]++;
		}
	}
