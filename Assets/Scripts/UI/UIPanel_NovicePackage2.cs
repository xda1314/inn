...
/// 新手礼包
/// </summary>
public class UIPanel_NovicePackage2 : UIPanelBase
{
    #region 组件
    ...
    private void RefreshData()
    {
        ...
        TryGetRemoteData();//远程配置覆盖本地数据
    }
    private void TryGetRemoteData()
    {
        string jsonStr = RemoteConfigSystem.Instance.GetRemoteConfig_String(RemoteConfigSystem.remoteKey_StarterPackage);
        if (string.IsNullOrEmpty(jsonStr))
            return;//未获取远程配置

        Dictionary<string, object> resultDic = JsonConvert.DeserializeObject<Dictionary<string, object>>(jsonStr);
        if (resultDic.TryGetValue("payid", out object payid_Json))
        {
            m_GiftDefinition.UnityID = payid_Json.ToString();
        }
        if (resultDic.TryGetValue("items", out object items_Json))
        {
            List<string> itemList = JsonConvert.DeserializeObject<List<string>>(items_Json.ToString());
            m_GiftDefinition.CoinOrGemRewardList.Clear();
            m_GiftDefinition.ItemRewardList.Clear();
            for (int i = 0; i < itemList.Count; i++)
            {
                MergeRewardItem item = new MergeRewardItem();
                string[] strs = itemList[i].Split(',');
                if (strs.Length == 2 && int.TryParse(strs[1], out int num))
                {
                    item.name = strs[0];
                    item.num = num;
                    if (item.IsRewardGems)
                    {
                        m_GiftDefinition.CoinOrGemRewardList.Add(item);
                    }
                    else
                    {
                        m_GiftDefinition.ItemRewardList.Add(item);
                    }
                }
            }
        }
    ...
    private void RefreshPackageUI()
    {
        ...
        for (int i = 0; i < itemList.Count; i++)
        {
            if (i >= uINovicePackageItems.Count)
                return;//最多显示六个
        ...
        for (int i = 0; i < uINovicePackageItems.Count; i++)
        {
            uINovicePackageItems[i].gameObject.SetActive(i < itemList.Count);
        }
        //强刷布局
