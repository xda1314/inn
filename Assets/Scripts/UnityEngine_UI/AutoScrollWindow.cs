...
public class AutoScrollWindow : UIBehaviour, IBeginDragHandler, IInitializePotentialDragHandler, IDragHandler, IEndDragHandler, ICanvasElement
{
    ...
    private Vector3[] tempVectors = new Vector3[4];
    ...
    public void AddEssentialComponent()
    {
    ...
    public void Clear()
    {
        int childCount = viewRectTran.transform.childCount;
        for (int i = childCount-1; i >=0; i--)
        { 
            DestroyImmediate(viewRectTran.transform.GetChild (i).gameObject);
        }
        m_index = 0;
        m_preIndex=0;
    }
   ...
   private bool IsCanAutoLoop()
    {
        return viewRectTran.transform.childCount > 1;
    }
    protected virtual void Update()
    {
        ...
        if (ContentIsLongerThanRect())
        {
            ...
            //自动loop
            if (AutoLoop && !m_IsNormalizing && EnsureListCanAdjust()&& IsCanAutoLoop())
            {
    ...
    public virtual void OnDrag(PointerEventData eventData)
    {
        ...
        if (!Drag || !contentCheckCache|| childCount<=1)
        {
            return;
        }
        Vector2 vector;
    ...
    public virtual void OnEndDrag(PointerEventData eventData)
    {
        ...
        if (((eventData.button == PointerEventData.InputButton.Left) && this.IsActive()) && RectTransformUtility.ScreenPointToLocalPointInRectangle(this.viewRectTran, eventData.position, eventData.pressEventCamera, out vector))
        {
            distance = vector.x - this.onBeginPos.x;
            if (distance > 0)
                isRight = true;
            else
                isRight = false;
    ...
    public virtual int GetBoundaryState()
    {
        ...
        left.GetWorldCorners(tempVectors);
        var leftX = tempVectors[0].x;
        var leftY = tempVectors[0].y;
  
        right.GetWorldCorners(tempVectors);
        var rightX = tempVectors[3].x;
        var rightY = tempVectors[1].y;


        viewRectTran.GetWorldCorners(tempVectors);
        var minRectX = tempVectors[0].x;
        var minRectY = tempVectors[0].y;
        var maxRectX = tempVectors[3].x;
        var maxRectY = tempVectors[2].y;
    ...
    {
        int childCnt = viewRectTran.childCount;
        for(int i = 0; i < childCnt; i++)
        {
            var child = viewRectTran.GetChild(i);
            child.localPosition += (Vector3)delta;
        }
    }
    public enum Axis
    {
