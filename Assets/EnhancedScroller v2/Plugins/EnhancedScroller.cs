...
namespace EnhancedUI.EnhancedScroller
{
    ...
    public class EnhancedScroller : MonoBehaviour
    {
        ...
        public int PreloadStartCount
        {
            get
            {
                return (_delegate != null ? _delegate.GetPreloadStart() : 0);
            }
        }

        public int PreloadEndCount
        {
            get
            {
                return (_delegate != null ? _delegate.GetPreloadEnd() : 0);
            }
        }
        ...
        public EnhancedScrollerCellView GetActiveCellViewsByDataIndex(int dataIndex)
        {
            for (var i = 0; i < _activeCellViews.Count; i++)
            {
        ...
        private void _ResetVisibleCellViews()
        {
            ...
            if (PreloadStartCount > 0)
            {
                if (startIndex - PreloadStartCount > 0)
                    startIndex -= PreloadStartCount;
                else
                    startIndex = 0;
            }
            if (PreloadEndCount > 0)
            {
                if (endIndex + PreloadEndCount < NumberOfCells)
                    endIndex += PreloadEndCount;
                else
